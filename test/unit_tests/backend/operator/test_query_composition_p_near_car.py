...
def person_close_to_car(car_value, person_value):
    car_tlbr = car_value["tlbr"]
    person_tlbr = person_value["tlbr"]
    person_center = (person_tlbr[0] + person_tlbr[2]) / 2, (person_tlbr[1] + person_tlbr[3]) / 2
    car_center = (car_tlbr[0] + car_tlbr[2]) / 2, (car_tlbr[1] + car_tlbr[3]) / 2
    return distance(person_center, car_center) < 100
...
def test_stateful_projector(tracker):
    ...
    node = FrameProjector(
        prev=node,
        property_name="car_near_person",
        property_func=person_close_to_car,
        dependencies=[((1, "car"), {"tlbr": 0}),
                      ((0, "person"), {"tlbr": 0})]
    )
    ...
    node = FrameFilter(
        prev=node,
        condition_func=lambda frame: frame.properties["car_near_person"],
        )
